...
export const TablePrice = () => {
    ...
    useEffect(() => {
        const tempInvoiceSubtotal = orderItems.reduce((accumulator: any, item: any) => accumulator + item.total_price, 0);
    ...
    const handleChangeQuantity = async (_event: any, orderItem: any,  value: any) => {
        try {
            ...
            if (value) {
                setOrderItems((prevOrderItems: any[]) => {
                    const updatedOrderItems = prevOrderItems.map(item =>
                        item.id === orderItem.id ? { ...item, quantity: value, total_price: value * item.unit_price } : item
                    );
                            ...
                            <TableRow key={orderItem.id}>
                                ...
                                <TableCell align="right" sx={{maxWidth: 2}}>
                                    <NumberInput
                                        ...
                                        onChange={(event, value) => handleChangeQuantity(event, orderItem, value)}
                                        readOnly
                                    />
                                </TableCell>
                                <TableCell align="right">${ccyFormat(orderItem.total_price)}</TableCell>
                            </TableRow>
                        )) : null
                    }
                    <TableRow>
